...
ENCOURAGING_PHRASES = [
    "Пусть день будет светлым и спокойным ☀️",
    "Хорошего дня и тепла в доме 💛",
    "Пусть всё получится сегодня! ✨",
    "Улыбок и добрых новостей 🌿",
    "Пусть в доме будет уют и радость 🤍",
    "Хорошего настроения и лёгкого дня 🙂",
]
...
def build_prompt(
    ...
    return textwrap.dedent(
        ...
        Никакой политики, тревожных тем, криминала, судов и негатива.
        ...
        5) История: строка «История: … (год)», если есть (позитивное историческое событие, ОБЯЗАТЕЛЬНО с годом)
...
def build_fallback_post(
    ...
    if history_event:
        ...
        if history_event and not re.search(r'\d{4}', history_event):
            history_event = f"{history_event} (год уточняется)"
